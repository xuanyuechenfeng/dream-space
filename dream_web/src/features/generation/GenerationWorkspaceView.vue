...
const hasDetail = computed(() => Boolean(active.value || pendingSubmission.value));
    ...
    <main class="generation-page" :class="{ 'sidebar-collapsed': collapsed, 'mobile-history-open': mobileHistoryOpen }">
      ...
      <section class="generation-main" :class="{ 'is-empty': !hasDetail }" :aria-hidden="mobileHistoryOpen ? 'true' : undefined" :inert="mobileHistoryOpen">
        ...
        <div ref="timeline" class="timeline">
          ...
          <template v-else>
            <div v-if="active && sortedTasks.length === 0 && !pendingSubmission" class="generation-empty">{{ text.noTasks }}</div>
              ...
              <article v-for="task in sortedTasks" :key="task.id" class="task">
                ...
                <div class="task-conversation">
                  ...
                  <section class="task-output" :class="{ 'has-single-result': task.imageCount === 1, 'has-multiple-results': task.imageCount > 1 }" :aria-label="text.output">
                    ...
                    <template v-if="task.slots?.length"><div v-if="task.imageCount > 1" class="collection-summary"><span>{{ task.successfulCount ?? task.results.length }} / {{ task.imageCount }} {{ text.output }}</span><span>{{ task.consumedCost ?? 0 }} {{ text.credits }}</span></div><div class="collection-slots" :class="{ 'is-single': task.imageCount === 1, 'is-multi': task.imageCount > 1 }"><div v-for="slot in task.slots" :key="slot.index" class="collection-slot" :class="`is-${slot.status}`"><div class="collection-slot-label"><strong>{{ slot.label }}</strong><span class="collection-slot-status">{{ slotStatusLabel(slot.status, task.status) }}</span></div><template v-if="slot.result"><button class="result-preview" type="button" :aria-label="text.preview" @click="previewUrl = assetUrl(slot.result.contentUrl)"><img :src="assetUrl(slot.result.contentUrl)" :alt="slot.label" :width="slot.result.width" :height="slot.result.height" decoding="async" @load="scrollTimelineToEnd" /></button><button class="result-download" type="button" :aria-label="text.download" :title="text.download" @click="download(slot.result.contentUrl, slot.result.id)"><Download aria-hidden="true" /></button></template><span v-else-if="slot.errorMessage" class="collection-slot-status">{{ slot.errorMessage }}</span><LoaderCircle v-else-if="slot.status === 'generating'" class="spin" aria-hidden="true" /></div></div></template>
            ...
            <article v-if="pendingSubmission" class="task pending-task" :class="{ 'is-failed': pendingSubmission.status === 'failed' }">
              <header class="task-card-header"><span class="task-agent"><span class="agent-dot" />{{ text.assistant }}</span><span class="task-meta"><span>{{ pendingSubmission.ratio }} · {{ pendingSubmission.resolution }} · {{ pendingSubmission.width && pendingSubmission.height ? `${pendingSubmission.width} x ${pendingSubmission.height}` : text.aiDimensions }}</span></span></header>
              <div class="task-conversation"><section class="task-input" :aria-label="text.input"><div class="message-label user-message-label">{{ text.input }}</div><div class="input-bubble"><p class="prompt-copy">{{ pendingSubmission.prompt }}</p><span class="task-params">{{ pendingSubmission.imageCountMode === 'AUTO' ? text.determiningCount : `${pendingSubmission.imageCount} ${text.imageUnit}` }} · {{ pendingSubmission.ratio }} · {{ pendingSubmission.resolution }}</span></div></section><section class="task-output" :aria-label="text.output"><div class="message-label assistant-message-label">{{ text.output }}</div><div v-if="pendingSubmission.status === 'preparing'" class="generation-placeholder pending-placeholder"><div class="placeholder-grid" aria-hidden="true"><span /><span /><span /><span /></div><div class="placeholder-spinner"><LoaderCircle class="spin" aria-hidden="true" /></div><span class="placeholder-status">{{ pendingSubmission.imageCountMode === 'AUTO' ? text.determiningCount : text.preparing }}</span><span class="placeholder-dimensions">{{ pendingSubmission.ratio }} · {{ pendingSubmission.resolution }}</span></div><div v-else class="generation-placeholder is-unavailable is-failed"><AlertCircle aria-hidden="true" /><span>{{ pendingSubmission.error }}</span></div></section></div>
              <div v-if="pendingSubmission.status === 'failed'" class="task-actions"><button class="action-btn" type="button" @click="retryPending">{{ text.retry }}</button><button class="action-btn" type="button" @click="editPending">{{ text.edit }}</button></div>
            </article>
          </template>
        </div>

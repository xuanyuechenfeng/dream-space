...
import { AlertCircle, ArrowUp, Check, ChevronDown, ChevronUp, Clock3, Download, Image, Link, LoaderCircle, Maximize2, Menu, PanelLeftClose, Pause, Pencil, Plus, SlidersHorizontal, SquarePen, Trash2, Unlink, X } from "lucide-vue-next";
...
import { api, type GenerationCountMode, type GenerationDraft, type GenerationRatio, type GenerationResolution, type GenerationResult, type GenerationSessionSummary, type GenerationTask } from "@/api/client";
...
function openPreview(result: GenerationResult) { previewResult.value = result; previewOriginal.value = false; }
function viewOriginal() { previewOriginal.value = true; }
function closePreview() { previewResult.value = null; previewOriginal.value = false; }
...
<template>
  <InspirationShell active-page="generate">
    <main class="generation-page" :class="{ 'sidebar-collapsed': collapsed, 'mobile-history-open': mobileHistoryOpen }">
              ...
              <article v-for="task in sortedTasks" :key="task.id" class="task">
                ...
                <div class="task-conversation">
                  ...
                  <section class="task-output" :class="{ 'has-single-result': task.imageCount === 1, 'has-multiple-results': task.imageCount > 1 }" :aria-label="text.output">
                    ...
                    <div v-else-if="task.results.length" class="result-grid" :class="{ 'is-single': task.imageCount === 1, 'is-square': task.ratio === '1:1', 'is-multi': task.imageCount > 1 }"><div v-for="result in task.results" :key="result.id" class="result-item"><button class="result-preview" type="button" :aria-label="text.preview" @click="openPreview(result)"><CachedImage :src="assetUrl(resultPreviewUrl(result))" :alt="text.output" :width="result.width" :height="result.height" @load="scrollTimelineToEnd" /></button><button class="result-download" type="button" :aria-label="text.download" :title="text.download" @click="download(result.contentUrl, result.id)"><Download aria-hidden="true" /></button></div></div>
      ...
      <div v-if="previewResult" class="image-preview" role="dialog" aria-modal="true" @mousedown.self="closePreview"><CachedImage :src="assetUrl(resultDialogUrl(previewResult, previewOriginal))" alt="Generated result preview" /><button class="icon-btn image-preview-close" type="button" aria-label="Close" @click="closePreview"><X aria-hidden="true" /></button><div class="image-preview-actions"><button v-if="!previewOriginal" class="icon-btn" type="button" :aria-label="text.viewOriginal" :title="text.viewOriginal" @click="viewOriginal"><Maximize2 aria-hidden="true" /></button><button class="icon-btn" type="button" :aria-label="text.download" :title="text.download" @click="download(previewResult.contentUrl, previewResult.id)"><Download aria-hidden="true" /></button></div></div>
    </main>
  </InspirationShell>
</template>

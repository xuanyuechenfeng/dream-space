...
import { api, type InspirationPage } from "@/api/client";
import { fixturePage } from "@/api/fixtures";
...
async function load() { controller?.abort(); controller = new AbortController(); loading.value = true; error.value = false; const params = new URLSearchParams(); if (category.value !== "all") params.set("category", category.value); if (query.value.trim()) params.set("q", query.value.trim()); try { response.value = shuffle(await api.inspirations(params, controller.signal)); saveHistory(query.value); } catch (e) { if ((e as Error).name === "AbortError") return; try { response.value = shuffle(fixturePage(category.value === "all" ? undefined : category.value, query.value)); } catch { error.value = true; } } finally { loading.value = false; } }
...
<template>
  <InspirationShell>
    ...
    <section v-else class="masonry" aria-label="Inspiration works"><RouterLink v-for="item in response?.items" :key="item.id" class="art-card" :to="`/inspiration/${item.slug}`"><img :src="item.thumbnailUrl" :alt="item.title" :width="item.width" :height="item.height" loading="lazy" /><span class="art-overlay"><span class="art-meta"><span class="art-title">{{ item.title }}</span><span class="art-author">{{ item.authorDisplayName }}</span></span><span class="same-chip">{{ text.same }}</span></span></RouterLink></section>
  </InspirationShell>
</template>

...
const accountOpen = ref(false); const themeOpen = ref(false); const legalOpen = ref(false); const loggingOut = ref(false); const watermark = ref(true);
...
const text = computed(() => language.value === "zh" ? { inspiration: "灵感", generate: "生成", mine: "我的", notification: "通知", settings: "设置", legal: "平台协议", changelog: "更新日志", appearance: "外观", watermark: "AI 水印", login: "登录", logout: "退出登录", quota: "创作额度", cycle: "本周期剩余", remaining: "剩余 80%", cost: "每张预计 1 点" } : { inspiration: "Explore", generate: "Create", mine: "Account", notification: "Notifications", settings: "Settings", legal: "Platform terms", changelog: "Changelog", appearance: "Appearance", watermark: "AI watermark", login: "Sign in", logout: "Sign out", quota: "Creation quota", cycle: "Remaining this cycle", remaining: "80% remaining", cost: "About 1 credit per image" });
...
onMounted(() => { window.addEventListener("mousedown", closeMenus); window.addEventListener("keydown", closeMenus); });
  ...
  <div class="app" :data-language="language">
    <nav class="primary-nav" :aria-label="language === 'zh' ? '主导航' : 'Primary navigation'">
      ...
      <div class="desktop-dock">
        <button class="dock-icon dock-tooltip" type="button" :aria-label="text.notification" :data-tooltip="text.notification"><Bell aria-hidden="true" /></button>
    ...
    <section v-if="accountOpen" class="account-menu" aria-label="Account and settings" @mousedown.stop>
      ...
      <button class="menu-row" type="button"><ScrollText aria-hidden="true" />{{ text.changelog }}<span class="menu-end">v0.1</span></button>
      ...
      <section v-if="auth.session?.authenticated" class="quota-panel" aria-label="Creation quota"><div class="quota-heading"><Sparkles aria-hidden="true" /><span class="quota-title"><strong>{{ text.quota }}</strong><small>{{ text.cycle }}</small></span><strong class="quota-value">80 / 100</strong></div><div class="quota-track"><span style="width: 80%" /></div><div class="quota-meta"><span>{{ text.remaining }}</span><span>{{ text.cost }}</span></div></section>

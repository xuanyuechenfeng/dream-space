...
| `AdminUser`             | `id`, `phone`, `displayName`, `role`, `active`, `status`, `version`, `lastLoginAt`, `createdBy`, `disabledAt`, `disabledBy`, `disabledReason`, `permissionRevision`                                                                                                                                                                           | `phone` 唯一；状态/角色/时间索引；状态与兼容 active 一致性约束                                    | 管理员账号；支持邀请、启停、版本控制和禁用审计 |
| `AdminRoleDefinition`   | `id`, `code`, `name`, `description`, `system`, `status`, `version`                                                                                                                                                                                                                       | `code` 唯一；系统角色保护 |
| `AdminPermissionDefinition` | `id`, `code`, `resource`, `action`, `description`, `riskLevel`, `status`                                                                                                                                                                                                              | `code` 唯一；权限码和风险等级约束 |
| `AdminUserRole`          | `adminUserId`, `roleId`, `assignedBy`, `assignedAt`                                                                                                                                                                                                                                    | 组合唯一；管理员/角色外键 |
| `AdminRolePermission`     | `roleId`, `permissionId`, `grantedBy`, `grantedAt`                                                                                                                                                                                                                                    | 组合唯一；角色/权限外键 |
| `AdminOperationIdempotency` | `scope`, `idempotencyKey`, `subjectId`, `createdAt`                                                                                                                                                                                                                                  | 组合唯一；创建时间索引 |

...
public interface GenerationMapper {
  ...
  @Select("SELECT * FROM \"GenerationPlan\" WHERE \"taskId\" = #{taskId} LIMIT 1")
  GenerationPlanRecord findPlan(String taskId);
  @Insert("INSERT INTO \"GenerationPlan\" (\"id\",\"taskId\",\"schemaVersion\",\"status\",\"inputHash\",\"requirementJson\",\"structureJson\",\"visualJson\",\"promptJson\",\"createdAt\",\"updatedAt\") VALUES (#{id},#{taskId},#{schemaVersion},#{status}::\"GenerationPlanStatus\",#{inputHash},CAST(#{requirementJson} AS JSONB),CAST(#{structureJson} AS JSONB),CAST(#{visualJson} AS JSONB),CAST(#{promptJson} AS JSONB),CURRENT_TIMESTAMP,CURRENT_TIMESTAMP) ON CONFLICT (\"taskId\") DO UPDATE SET \"status\"=EXCLUDED.\"status\",\"requirementJson\"=EXCLUDED.\"requirementJson\",\"structureJson\"=EXCLUDED.\"structureJson\",\"visualJson\"=EXCLUDED.\"visualJson\",\"promptJson\"=EXCLUDED.\"promptJson\",\"updatedAt\"=CURRENT_TIMESTAMP")
  int upsertPlan(@Param("id") String id, @Param("taskId") String taskId, @Param("schemaVersion") String schemaVersion,
      @Param("status") String status, @Param("inputHash") String inputHash, @Param("requirementJson") String requirementJson,
      @Param("structureJson") String structureJson, @Param("visualJson") String visualJson, @Param("promptJson") String promptJson);
  @Select("SELECT * FROM \"GenerationIteration\" WHERE \"taskId\" = #{taskId} ORDER BY \"iteration\" ASC")
  java.util.List<GenerationIterationRecord> listIterations(String taskId);
  @Insert("INSERT INTO \"GenerationIteration\" (\"id\",\"taskId\",\"iteration\",\"promptHash\",\"status\",\"provider\",\"model\",\"providerRequestId\",\"evaluationJson\",\"refinementJson\",\"errorCode\",\"startedAt\",\"completedAt\") VALUES (#{id},#{taskId},#{iteration},#{promptHash},#{status}::\"GenerationIterationStatus\",#{provider},#{model},#{providerRequestId},CAST(#{evaluationJson} AS JSONB),CAST(#{refinementJson} AS JSONB),#{errorCode},CURRENT_TIMESTAMP,#{completedAt}) ON CONFLICT (\"taskId\",\"iteration\") DO UPDATE SET \"status\"=EXCLUDED.\"status\",\"evaluationJson\"=EXCLUDED.\"evaluationJson\",\"refinementJson\"=EXCLUDED.\"refinementJson\",\"errorCode\"=EXCLUDED.\"errorCode\",\"completedAt\"=EXCLUDED.\"completedAt\"")
  int upsertIteration(@Param("id") String id, @Param("taskId") String taskId, @Param("iteration") int iteration,
      @Param("promptHash") String promptHash, @Param("status") String status, @Param("provider") String provider,
      @Param("model") String model, @Param("providerRequestId") String providerRequestId,
      @Param("evaluationJson") String evaluationJson, @Param("refinementJson") String refinementJson,
      @Param("errorCode") String errorCode, @Param("completedAt") java.time.Instant completedAt);
}

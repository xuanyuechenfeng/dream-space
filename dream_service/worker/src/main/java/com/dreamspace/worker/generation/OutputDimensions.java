...
public record OutputDimensions(int width, int height) {
  public static OutputDimensions resolve(GenerationRatio ratio, GenerationResolution resolution) {
    return resolve(ratio, resolution, null, null);
  }

  public static OutputDimensions resolve(GenerationRatio ratio, GenerationResolution resolution, Integer width, Integer height) {
    if (width != null && height != null) return new OutputDimensions(width, height);
    ...
    int[] parts = switch (ratio) {
      ...
      case CUSTOM -> new int[] {1, 1};
    };
    if (parts[0] >= parts[1]) return new OutputDimensions(edge, roundTo64((float) edge * parts[1] / parts[0]));
    return new OutputDimensions(roundTo64((float) edge * parts[0] / parts[1]), edge);
  }

  private static int roundTo64(float value) { return Math.max(64, Math.round(value / 64f) * 64); }
}
